...
app.get("/wizards", checkRoleToken, (req, res) => {
  ...
  const query = `
    SELECT
      ...
      json_agg(
        role.*
      ) OVER (PARTITION BY wizard.id) as roles
    FROM WizardRole
      JOIN Role ON WizardRole.role_id = Role.id
      JOIN wizard ON wizard.id = WizardRole.wizard_id
    ORDER BY wizard.firstname, wizard.lastname ASC
...
app.post("/subjects", (req, res) => {
  ...
  client.query(
    "INSERT INTO subject (name, description, image, room_id, wizar_id) VALUES ($1, $2, $3, $4, $5) RETURNING *",
    [subject.name, subject.description, subject.image, subject.room_id, subject.wizar_id]
  ).then((result) => {
    res.status(201).json(result.rows[0]);
  })
  .catch(error => {
...
app.put("/subjects/:id", (req, res) => {
  ...
  const subject = req.body;
  client.query(
    "UPDATE subject SET name=$1, description=$2, image=$3, room_id=$4, wizar_id=$5 WHERE id=$6 RETURNING *",
    [subject.name, subject.description, subject.image, subject.room_id, subject.wizar_id, id]
  ).then((result) => {
    res.status(200).json(result.rows[0]);
  })
  .catch(error => {
